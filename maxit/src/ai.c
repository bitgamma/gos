...
int16_t mxt_ai_minmax(mxt_maxit_t* maxit, uint8_t row_col, int16_t score, uint8_t depth, int16_t alpha, int16_t beta, bool maximize, uint8_t* max_row) {
  if (depth == 0) {
    return score;
  }
  ...
  if (maximize) {
    ...
    for (int i = 0; i < BOARD_SIZE; i++) {
      ...
      int16_t t = mxt_ai_minmax(maxit, i, score + maxit->game.board.board[i][row_col], depth - 1, alpha, beta, false, max_row);
      if (t > val) {
        val = t;
        *max_row = i;
      }  
    ...
    for (int i = 0; i < BOARD_SIZE; i++) {
      ...
      val = min(val, mxt_ai_minmax(maxit, i, score - maxit->game.board.board[row_col][i], depth - 1, alpha, beta, true, max_row));
...
void mxt_ai_move(mxt_maxit_t* maxit) {
  ba_clear(_moves[maxit->game.board.cursor_row], maxit->game.board.cursor_column);
  mxt_ai_minmax(maxit, maxit->game.board.cursor_column, 0, mxt_ai_get_depth(maxit), MINF, PINF, true, &maxit->game.board.cursor_row);
  ba_clear(_moves[maxit->game.board.cursor_row], maxit->game.board.cursor_column);
}